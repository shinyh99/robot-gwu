...
class MGeoPlannerMap():
    ...
    sign_set=SignalSet(), light_set=SignalSet(), synced_light_set=None, intersection_controller_set=None, sm_set=None):
        '''
        ...
        if supported_class['surface_marking']:
            if intersection_controller_set is None:
                self.sm_set = SurfaceMarkingSet()
            else:
                self.sm_set = surface_sm_setmarking
        else:
            self.sm_set = None
