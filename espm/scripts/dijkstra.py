...
class Dijkstra:
    ...
    # 가중치 입력 행렬
    def get_weight_metrix(self) -> dict:
        # 초기 설정
        weight = dict()
        ...
        for from_node_id, from_node in self.nodes.items():
            ...
            for to_node in from_node.get_to_nodes():
                (
                    shortest_link,
                    min_cost,
                ) = from_node.find_shortest_link_leading_to_node(to_node)
    ...
    def find_nearest_node_idx(self, distance, s):
        ...
        if sys.version_info[0] >= 3:  # for python3
            ...
            min_idx = idx_list_subscriptable[
                -1
            ]  # NOTE: 매우 중요. 의미 없는 값이 아님. None을 줘버리면 오동작한다.
        else:  # for python2
            min_idx = idx_list[
                -1
            ]  # NOTE: 매우 중요. 의미 없는 값이 아님. None을 줘버리면 오동작한다.
    ...
    def draw_lane_change(start_link, end_link, lane_change_distance, step_size):
        output_path = []
        translation = [start_link.points[0][0], start_link.points[0][1]]
        theta = atan2(
            start_link.points[1][0] - start_link.points[0][1],
            start_link.points[1][0] - start_link.points[0][0],
        )
        ...
        # Draw path changing the lane
        for i in range(len(y)):
            local_change_path = np.array([[x[i]], [y[i]], [1]])
            global_change_path = t.dot(local_change_path)
            output_path.append(
                [global_change_path[0][0], global_change_path[1][0], 0]
            )

        # Find the first point on the last link
        end_point_index = 0
        for (i, end_point) in enumerate(local_end_link_matrix.tolist()):
            if (
                end_point[0] == local_end_point_matrix[0][0]
                and end_point[1] == local_end_point_matrix[1][0]
            ):
                end_point_index = i
                break

        # Draw points on the last link
    ...
    def find_shortest_path(self, start_node_idx, end_node_idx):
        ...
        for link_id in link_path:
            ...
            if user_option_draw_lane_change and link.is_it_for_lane_change():
                ...
                output_path = Dijkstra.draw_lane_change(
                    lane_ch_first["from"],
                    lane_ch_last["to"],
                    lane_ch_distance,
                    1,
