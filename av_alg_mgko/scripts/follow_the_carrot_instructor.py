...
class followTheCarrot :
    def __init__(self):
        ...
        rospy.Subscriber("ctrl_collision", CollisionAvoidance, self.avoidance_callback) 
        ...
        self.avoidance_data = CollisionAvoidance()
        ...
        while not rospy.is_shutdown():
            ...
            if self.is_path ==True and self.is_odom==True and self.is_target_vel  :
                ...
                if self.is_look_forward_point :
                    ...
                    if self.is_ca:
                        tmpTheta = theta
                        print('orig', theta/pi*180)
                        tmpData = self.avoidance_data
                        tmpNum = (tmpData.ca_const_alpha/tmpData.ca_distance*tmpData.phi_gap) + (tmpData.ca_const_beta*tmpTheta)
                        tmpDen = (tmpData.ca_const_alpha/tmpData.ca_distance) + tmpData.ca_const_beta
                        theta = tmpNum/tmpDen
                    print('calced', theta/pi*180)
                    self.ctrl_msg.angular.z=-theta * 3
                    self.ctrl_msg.linear.x=self.target_vel * (0.5 + 0.5*np.cos(np.abs(theta)))
