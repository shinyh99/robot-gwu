...
class MapProc:
    ...
    def callbackOccuGrid(self, msg):
        ...
        _, _, tmpYaw = euler_from_quaternion(
...
if __name__ == "__main__":
    ...
    mapProc = MapProc()
    rospy.Subscriber(
        "OccupancyGrid", OccupancyGrid, mapProc.callbackOccuGrid, queue_size=1
    )
    ...
    while not rospy.is_shutdown():
        ...
        tmpMapData = np.reshape(
            tmpMap.data, (tmpMap.info.width, tmpMap.info.height)
        )
        tmpMapDataRot = rotate(
            tmpMapData,
            angle=mapProc.egoHeading / math.pi * 180 + 180,
            reshape=False,
        )
        tmpMapDataRot = np.fliplr(tmpMapDataRot)
        tmpMapDataRot = tmpMapDataRot.astype(np.int8)
        tmpMapDataRot = np.clip(tmpMapDataRot, 0, 100)
        tmpMapTot = np.reshape(
            tmpMapDataRot, (tmpMap.info.width * tmpMap.info.height)
        )
