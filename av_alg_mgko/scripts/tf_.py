...
class LocationSensor:
    def __init__(self, zone=52):
        ...
        self.x, self.y, self.heading, self.velocity, self.gps_status = (
            None,
            None,
            None,
            None,
    ...
    def convertLL2UTM(self, lat: float, lon: float):
        ...
        z = np.complex(
            ...
            np.log(
                np.tan(
                    np.pi / 4.0
                    + np.arcsin(np.sin(l1 - self.L0) / np.cosh(L)) / 2.0
                )
            ),
        )

        Z = self.N * self.C2[0] * z + self.N * (
            self.C2[1] * np.sin(2.0 * z)
