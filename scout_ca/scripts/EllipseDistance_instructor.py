...
def EllipseDistance(semi_major, semi_minor, rotation, point):
    ...
    return [round(np.copysign(x, point[0]), 6), round(np.copysign(y, point[1]), 6)]
...
def EllipseEndPoint(semi_major, semi_minor, rotation, point):
    ...
    tmpAlphaTerm = zeta4*delta4 - 4*(zeta4*delta3*px2 + delta4*zeta3*py2 - delta4*zeta3*px2*py2 - delta5*zeta2*py4)
